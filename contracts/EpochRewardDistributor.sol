...
/// @notice Top-performer bonus rewards by AIU score. Inspired by Bittensor emission-to-top-miners model. Distributes bonus INTEL to highest-AIU workers at each epoch end.
///
...
contract EpochRewardDistributor {
    ...
    function distributeEpochRewards(uint256 epoch) external onlyOperator nonReentrant {
        ...
        bool ok = intel.transferFrom(treasury, address(this), pool);
        require(ok, "EpochRewardDistributor: transferFrom failed");
